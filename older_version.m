...
calculateInsertionProb = 0;

calculateDeletionProb = 0; %1 Calculate P1 and P0.  otherwise used fixed values
...
if demoCode == 2 
   ...
   probInsertion = zeros(samples,Kmax)-5.432;
   probDeletion  = zeros(samples,Kmax)-5.432;
   
   K = Kguess;  Kactual = Ktrue;
   Total = T;
   %T is from original file
   phi = Kactual/T; %The algorithm should actually not be sensitive to the choice of phi
   ...
   kMax = 2*Kmax; %we will have a long vector of possible change points
   Kmax_original = Kmax;
   ...
   Svector(1,1:length(Sm)) = Sm; %Segment left endpoints (last element is right-most point)
   ...
   for i=1:(K-1)
        if i < (K-1)
            C(i) = mean(    Xdata(   Sm(i) : (Sm(i+1)-1)    )    )  ;
        else
            C(i) = mean(    Xdata(   Sm(i) : (Sm(i+1))    )    )  ;
        end
   end
        %insertion and deletion for each step
   C = C_initial;  %<<<<<<<<<<<<<<<<<
...
%% Gibbs Sampler
Svector(1,1:length(Sm)) = Sm;  

for rep = 1:samples
    K = length(Sm) - 1; %Estimated number of segments
    K_now = K;
    % Our "Sm" vector contains the left endpoints of the segments, 
                    % in addition to the last data point
    Snew = [1]; Cnew = C(1); Gnew = [1]; % List of new assignments for updating purposes
    
    if length(Sm) > 2    %<<<< possibly should change  to just evaluate Sm
        dLeft = Sm(1);  dPoint = Sm(2);  dRight = Sm(3);   
    end
    disp('.'); disp('.');
    ...
    for k = 1:(K_now - 1)
        ...
        % Insertion step       
        if (Sm(M+1) - ileft) > 1 % If we don't have adjacent points then we will do the insertion step
        %if (Sm(M+1) - ileft) > 1 % If we don't have adjacent points then we will do the insertion step
            z = ileft + ceil(rand()*( (Sm(k+1)-ileft)-1) ) - 1;
  ...
  %% proposed/randomly chosen new left and right interval means
            ...
            %number of currently estimated segments (number of moves)
            ...
            if calculateInsertionProb == 1
                acceptanceProb = Prob1/(Prob1+Prob0); %<-------- calculate this based on data
            else 
            ...
            if rand() < acceptanceProb
                insertionTrue = 1;
                statement = 'insertion occurs, Snew includes new z+1'; disp(statement);
                Snew = [Snew,z+1];
            ...
            statement = 'adjacent points, Snew not updated'; disp(statement);
            adjacentPoints = 1;
        end % mean updates
        ...
        if k < (K_now-1)  
            ...
            if calculateDeletionProb == 1
                boundDeletion = dProb0/(dProb0+dProb1); %<-------- calculate this based on data
            else 
                boundDeletion = P_delete_fixed;
            end
            disp(['Deletion Prob ',num2str(boundDeletion)])
            probDeletion(rep,k) = boundDeletion;
            if rand() < boundDeletion
                statement = 'Deletion occurs, deleted point not added to Snew'; disp(statement);
                disp(['Snew =  ',num2str(Snew)])
                
                %     
                %Sm(M)   Sm(M+1) = z+1     Sm(M+2)     S(length(Sm)) 
                
                Sm = [Sm(1:M),Sm((M+2):length(Sm))   ]; %<<<< changed
                ...
                statement = 'No deletion, Snew now contains non-deleted point'; disp(statement);
                Snew(length(Snew)+1) = Sm(k+1);
...
for i = 1:(length(Sm))
    xline(Sm(i),'linewidth',2,'color','blue')
end
estimateTitle = [num2str(length(Sm)-2),' Estimated Change Points'];
...
title([num2str(Ktrue-1),' Actual Change Points'], 'Interpreter','latex','FontSize',16)
xticks(Strue)
hold on
plot(Xdata)
